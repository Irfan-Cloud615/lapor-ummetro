...
var Helpers = {
  ...
  on: function on() {
    ...
    var _event$split3 = event.split('.'),
      ...
      namespace = _arrayLikeToArray(_event$split4).slice(1);
  ...
  off: function off() {
    ...
    var _event$split7 = event.split('.'),
      ...
      namespace = _arrayLikeToArray(_event$split8).slice(1);
    namespace = namespace.join('.') || null;
    this._listeners.filter(function (listener) {
      return listener.event === _event && listener.namespace === namespace;
    }).forEach(function (listener) {
      return _this0._listeners.splice(_this0._listeners.indexOf(listener), 1);
    });
  },
  // *******************************************************************************
  // * Life cycle
  init: function init() {
    ...
    this.on('init._Helpers', function () {
      _this1.off('resize._Helpers:redrawMenu');
      _this1.on('resize._Helpers:redrawMenu', function () {
        // eslint-disable-next-line no-unused-expressions
        _this1.isSmallScreen() && !_this1.isCollapsed() && _this1._redrawLayoutMenu();
      });

      // Force repaint in IE 10
      if (typeof document.documentMode === 'number' && document.documentMode < 11) {
        _this1.off('resize._Helpers:ie10RepaintBody');
        _this1.on('resize._Helpers:ie10RepaintBody', function () {
          if (_this1.isFixed()) return;
